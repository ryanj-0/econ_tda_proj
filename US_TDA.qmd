...
beaConfig <- list('UserID' = beaKey,
                  'Method' = 'GetData',
                  'datasetname' = 'NIPA',
                  'Year' = 'X',    # Get All Years Available
                  'ResultFormat' = 'json')
```
...
## Annual GDP
gdpA_list <- append(beaConfig, list('TableName' = 'T10101',
                                    'Frequency' = 'A'))
...
GDP_A <- beaGet(gdpA_list) %>%
  ...
  select(!c(time, UNIT_MULT))

## GDP Quartly  
gdpQ_list <- append(beaConfig, list('TableName' = 'T10101', 
                                    'Frequency' = 'Q'))
...
```{r}
#| label: Import and Format BEA - Exports and Imports %Change
#| warning: false

## Annual Exports/Imports
exImA_list <- append(beaConfig, list('TableName' = 'T40201',
                                     'Frequency' = 'A'))
exIm_A <- beaGet(exImA_list) %>%
  pivot_longer(cols = !(TableName:UNIT_MULT), 
               names_to = "time",
               values_to = "pctChange") %>%
  mutate(year = str_extract(time, "[0-9]{4}")) %>%
...
## Quarterly Exports/Imports
exImQ_list <- append(beaConfig, list('TableName' = 'T40201',
                                     'Frequency' = 'Q'))  
exIm_Q <- beaGet(exImQ_list) %>%
  pivot_longer(cols = !(TableName:UNIT_MULT), 
               names_to = "time",
               values_to = "pctChange") %>%
  mutate(year = str_extract(time, "[0-9]{4}"),
