...
The latter is a sub-field of Algebraic Topology which provides mathematical certainty about what we are seeing from the Mapper algorithm. The former, is a visualization tool and can be used for exploratory data analysis.
...
Additionally, BallMapper is less effective and inefficent to use when the input data variation there is large variation in the scales of each dimension of the dataset (columns). \footnote{[find reference]} To aide BallMapper, we transformed our data to either a rate or a percentage change from the previous year where appropriate. This ensures all dimensions of our data stay on a 0-100 scale and helps with the time complexity of BallMapper.

## *Pre Analysis Calculations*
The first data series we needed to to make adjustments for was Personal Income and It's Outlays; specifically, Personal Income and Its Disposition (Personal Income).
\footnote{Table 2.1 from the BEA's Interactive Data, National Data section.}
Personal Income is reported in nominal dollars, which does not account for inflation, so our first calculation is to adjust Personal Income for inflation using Consumer Price Index (CPI) data.
$$
\text{Real Personal Income} = \frac{\text{Personal Income}}{\text{CPI}} * 100
$${#eq-cpiRealDollars}
We then take the output os @eq-cpiRealDollars and take the log difference and multiply it by 100 to get a percentage change for the preceding year.
$$
\Delta\text{Real Personal Income} = ln(\text{Real Personal Income}_{t}) - ln(\text{Real Personal Income}_{t-1})
$${#eq-deltaPersoanlIncome}
 
 that came in monthly frequencies, two different calculations wered done based on the type of data (see @tbl-dataSources). Data labeled type *Flow* 
\footnote{This data is measuring some quantity over time such as Personal Income} and was not already given as a percent growth, we took a two step calculation: first we found the annual average by calculating the non-weighted arithmetic mean. After finding the 
