...
Since the late 19th Century, statistical techniques have been applied to the field of Economics. Common methodologies include generalized linear models or time series analysis. Although technology has advanced we still use these traditional methods but increasingly, new methods are being tested and developed.

The goal of this paper is examine the macroeconomic landscape of the United States of America (US) through a lens of Topological Data Analysis (TDA). Specifically, we are going to apply a TDA algorithm called BallMapper (TDABM) developed by Pawel Dlotko. This new algorithm simplifies traditional TDA methods by reducing the need for the user to pass data through multiple functions. Dlotoko demonstrates that, mathematically, TDABM produces a near similar to that of traditional TDA methods for certain dimensions via a fuctor map [@dlotko2019ballmapper]. Thus, one benefit of TDABM compared to the traditional methods is, for those who have the tools and introductory understanding in Topology or TDA, TDABM could be a good entry point to the field of TDA.

What follows below is primarily a proof-of-concept for TDA BallMapper and closely follows other papers that have applied TDABM. We will begin with a general overview of TDABM, current published literature, and useful concepts to know about TDABM. We then will describe out data and the reasoning behind their selection for analysis. Our [Methodology](#sec-methodology) and [Results](#sec-results) will touch on more mathematical concepts of TDABM and a couple of findings, respectively.

Our hope here is for the reader to walk away understanding the general motivation behind TDA BallMapper, how it differs from other common statistical methods, and how methods such as TDA are additive to our current body of mathematical tools.

[^supplements]: Supplementary materials referenced are available upon request via email: johnson.ryan1019@gmail.com.

# Literature
TDA is a relatively new field and is increasingly being applied to various areas of research such as genetics, financial fraud, biology, and image detection. From the small body of literature we have reviewed, TDABM has removed many of the parameters that are required for more traditional methods of TDA. Generally speaking, traditional TDA contains four broad steps, each with multiple user-defined parameters. Conversely, TDABM has reduced this process to an algorithm (FIXTHIS) and requires the user to select an epsilon value -- details on this follow below. What should be noted is that, although TDABM reduces the number of steps needed to produce similar outcomes of traditional TDA methods, we lose control of fine tuning of parameters. We also find that interpreting results becomes more difficult; however, TDABM is still in its infancy, so there is not a large body of research on interpretation [@dlotko2019ballmapper]. As mentioned earlier, this paper adds to the literature by providing another proof-of-concept case for TDABM. One major motivation for this paper is a paper written by @dlotko2019macroeconomy that applied TDABM to a global macroeconomic dataset to compare how countries have evolved over time, their transformation from the Great Depression Era, and various views on wealth and inequality. We, however, have not found any literature found specifically focused on the macroeconomic economy of singular countries; hence, our topic of choice.
...
rawData <- list.files(paste(getwd(), "data/raw", sep = "/"), recursive = TRUE)
for (f in rawData) {
    dT <- fread(paste(getwd(), "data/raw", f, sep = "/"))
    assign(sub("/", "_", f) |> str_remove(".csv"), dT)
}
```

```{r}

NEIA <- BEA_export_imports_A[c(1, 3, 27, 52:58), !1] %>%
  .[1,1:="catagory"] %>%
  setnames(paste(.[1])) %>%
  .[!1] %>%
  mutate(catagory = str_trim(catagory)) %>%
  mutate_if(is.numeric, as.character) %>%
  pivot_longer(!catagory, names_to = "year", values_to = "pctDelta") %>%
  pivot_wider(names_from = catagory, values_from = pctDelta) %>%
  mutate_if(is.character, as.numeric) %>%
  setnames(paste("neia", gsub(" ", "_", names(.)), sep = "")) %>%
  rename("year" = names(.)[1]) %>%
  mutate(source = "BEA_exportsImportsA")

NEIQ <- BEA_export_imports_Q[c(1:3, 27, 52:58), !1] %>%
  .[1,1:="catagory"] %>%
  .[2,1:="Quarter"] %>%
  setnames(paste(.[1], .[2], sep = "_")) %>%
  .[!1:2] %>%
  rename(catagory = catagory_Quarter) %>%
  mutate(catagory = str_trim(catagory)) %>%
  pivot_longer(!catagory, 
               names_to = c("year", "quarter"), names_sep = "_",
               values_to = "pctDelta") %>%
  pivot_wider(names_from = catagory, values_from = pctDelta) %>%
  mutate_at(c(1, 3:ncol(.)), as.numeric) %>%
  setnames(paste("neiq", gsub(" ", "_", names(.)), sep = "")) %>%
  rename("year" = names(.)[1]) %>%
  rename("quarter" = names(.)[2]) %>%
  mutate(source = "BEA_exports_imports_Q")
...
FE <- BEA_foreign_direct_invesment[, 2:11] %>%
  setnames(paste(.[3])) %>%
  .[!1:3] %>%
  pivot_longer(1:ncol(.), names_to = "year", values_to = "FE") %>%
  mutate_if(is.character, as.numeric)
  
PE <- BEA_foreign_direct_invesment[, 12:21] %>%
  setnames(paste(.[3])) %>%
  .[!1:3] %>%
  pivot_longer(1:ncol(.), names_to = "year", values_to = "PE") %>%
  mutate_if(is.character, as.numeric)

FDI <- merge(PE, FE, by = "year") %>%
  mutate(DE = PE-FE) %>%
  setnames(paste("fdi_", gsub(" ", "_", names(.)), sep = "")) %>%
  rename("year" = names(.)[1]) %>%
  mutate(source = "BEA_FDI")
...
GDPA <- BEA_gdp_A[!c(2, 16:17, 29:30), !1] %>%
  .[1, 1:="catagory"] %>%
  .[4, 1:= "Personal Goods"] %>%
  .[7, 1:= "Personal Services"] %>%
  .[16, 1:= "Export Goods"] %>%
  .[17, 1:= "Export Services"] %>%
  .[19, 2:= "Import Goods"] %>%
  .[20, 2:= "Import Services"] %>%
  setnames(paste(.[1])) %>%
  .[!1] %>%
  mutate(catagory = str_trim(catagory)) %>%
  pivot_longer(!catagory, names_to = "year", values_to = "pct_delta") %>%
  pivot_wider(names_from = catagory, values_from = pct_delta) %>%
  mutate_if(is.character, as.numeric) %>%
  setnames(paste("gdpa", gsub(" ", "_", names(.)), sep = "")) %>%
  rename("year" = names(.)[1]) %>%
  mutate(source = "BEA_gdp_A")


GDPQ <- BEA_gdp_Q[!c(16:17, 29:30), !1] %>%
  .[1, 1:="catagory"] %>%
  .[2, 1:="Q"] %>%
  .[5, 1:= "Personal Goods"] %>%
  .[8, 1:= "Personal Services"] %>%
  .[17, 1:= "Export Goods"] %>%
  .[18, 1:= "Export Services"] %>%
  .[20, 2:= "Import Goods"] %>%
  .[21, 2:= "Import Services"] %>%
  setnames(paste(.[1], .[2], sep = "_")) %>%
  .[!1:2] %>%
  ...
  pivot_wider(names_from = catagory, values_from = pct_delta) %>%
  mutate_at(c(1,3:ncol(.)), as.numeric) %>%
  setnames(paste("gdpq", gsub(" ", "_", names(.)), sep = "")) %>%
  rename("year" = names(.)[1]) %>%
  rename("quarter" = names(.)[2]) %>%
  mutate(source = "BEA_gdp_Q")
```
