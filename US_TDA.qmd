...
Generally speaking, traditional TDA contains four broad steps, each with multiple user-defined parameters. 

Conversely, TDABM has reduced this process to an the user selecting their data, a coloring variable and an epsilon value -- details on this follow below. What should be noted, although TDABM reduces the number of steps needed to produce similar outcomes of traditional TDA methods, we lose control of being able fine tune out outputs. 

We also find that interpreting results becomes more difficult. 

However, TDABM is still in its infancy, so there is not a large body of research on interpretation. [@dlotko2019ballmapper] 

One major motivation for this paper is a paper written by @dlotko2019macroeconomy that applied TDABM to a global macroeconomic dataset to compare how countries have evolved over time, their transformation from the Great Depression Era, and various views on wealth and inequality. We could not find other TDA literature found specifically focused on the macroeconomic economy of singular countries. Hence, our topic of choice.
...
Our next transformation was on Housing Starts, Producer Price Index - All Commodities (PPI) and CPI. We should note that PPI tracks only physical goods such as: Farm Products and Feed; Industrial Commodities; and Other Commodities such as Aircraft, Steel, and Lumber.[@bls_ppi_concepts]
...
# Asus: 59146, ThinkPad: 28716
ColorIgraphPlot(BM_Compare, seed_for_plotting = 59146)
bm_ggraph(BM_Compare2, coloring = coloring, epsilon = e)
```
Having the nodes spread out helps us see the structure of our graph more clearly as well as the new piece of information that was given to use but not see in @fig-newGraph-1, the edge strength. We see that in @fig-newGraph-2, highlighting the edge strength presents an insight about the relation between certain nodes. Further, out data consists of 65 rows, but one could imagine the advantage of a data set much larger that this one (e.g. looking at this data on a quarterly or monthly basis). Additionally, we changed the color palette for accessibility and readability. Looking at @fig-newGraph-1, any nodes colored in the royal blue (years approximately 2010-2015) mask the number identifier of the node.

For @fig-newGraph-2 and @fig-EDA, below, we use Year as our coloring variable. We started with this variable because a large motivation for this paper came from a paper by D\l{}otko, Rudkin, and Qiu,_Topologically Mapping the Macroeconomy_. In this paper was a two pronged macroeconomic analysis with Ball Mapper. The first was a dataset of five macroeconomic variables from  16 countries from the 1870s-2017. Coloring by Year they looked at the Ball Mapper graph structure as well as the the effect and recovery of the Great Depression on these countries.
The second prong was using Ball Mapper to look at relationship between private credit growth and GDP using Ball Mapper. This part of the paper they colored by various dimension within their point cloud as well as Year which was not included in the their point cloud.[@dlotko2019mapping]
