...
Looking at the time ranges in @tbl-dataSources, we elected to exclude Employment Cost Index (ECI) and Foreign Direct Investment (FDI) due to their limited available years. Looking at the remaining data series, Housing Starts has the smallest range and so , we will limit our years to the years 1959 - 2024. 

```{r}
#| label: tbl-dataSources
#| tbl-cap: "*New Privately-Owned Housing Units Started (Housing Starts) is considered a flow because it is provided as a Seasonally Adjusted Annual Rate (SAAR)."
...
@tbl-dataSources also shows that our data came in annual, quarterly, and monthly time frequencies. This analysis is using data by year so some considerations we had to account for was that not all our data provided in Real dollars, were given as a rate (growth rate, proportion rate,  or fixed rate), or on a similar scale.
BallMapper is less effective and harder to use when the input data variation is large regarding each dimension's sclae [find reference]. To fix this we transformed out data to either a rate or a percentage change from the previous year where appropriate.

only so for our data from FRED that came in monthly frequencies only we did two different calculations based on the type of data we had seen in our *Data Type* column. For data that is of type *Flow* - measuring some quantity over time - and was not already given as a percent growth, we first found the annual average if was given on a monthly or quarterly time frame using non-weighted arithmetic mean. Then with those averages
