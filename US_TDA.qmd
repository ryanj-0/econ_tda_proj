...
ggraph_Fisher
```
Focusing more on our connected components for now, we start by looking at C1. Since C1 has more than two nods, any nodes which are not directly connected by an edge (e.g. Nodes 24 and 4), can be interpreted as alike in some fashion, but also signal a significant difference within the component. Additionally, we know that the size of the node is commensurate with the number of points in contains.[@dlotko2019mapping]

 Additionally, we see that edge exists between nodes in our components, this means that node on either end of an edge contain at least one node. As mentioned above, we added the edge strength feature included in the original Ball Mapper algorithm output but not the traditional Ball Mapper graph. This is displayed in our new output by how thick a line appears between two nodes (e.g. nodes 4 and 29). One very interesting conclusion, and important to our analysis later, that D\l{}otko, et al. [@dlotko2019mapping] makes is that if two nodes within a component are not directly connected by one edge, they should be considered separate entities or parts of our point cloud. That is, nodes that make up a connected component have a large theme between all nodes in the component. Within a particular component, though, there are different parts to whatever throughline the component is picking up.

A nice relative measure one can use is that singletons are not just nodes by themselves but also usually only house one or two points. That said, we observe that C2 and C3 most likely house only a few points, albeit they are a connected components.

Finally, looking at the coloration of our Ball Mapper graph, the first things we look at are any particular patterning. For C1 we see some grouping withing the component, C2 it seems to be stable, and C3 there is some inconsistency. These smaller patches of color in C1 and the inconsistency in C3 are something to take note about. However, it is important to take into consideration the variable you are coloring by and how the Ball Mapper algorithm colors by the average value. Overall, though, there is not significant pattern for this particular coloring. We should also add that if you color by the different dimensions of the point cloud, dimensions that are highly correlated will end up with similar coloring patterns. Conversely, negatively correlated dimensions show opposite coloring.[@dlotko2019mapping]

Looking at @fig-EDA, we first observe the overall structure of our graph. There is one large connected component, one small component, and the rest singleton nodes. For the largest connected component (C1), we immediately are interested in the three large nodes (4, 24, and 26) because they are the largest in size for C1 and the same for their edge thickness (edge strength). Additionally, we also notice that each of the three large nodes have at least one flare (node connected only to themselves) as well as sharing others. What this structure tells us is, there are three distinct groups and each flare representing similar data to that specific group but perhaps is distinctly different to the other flares.
