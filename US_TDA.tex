...
\setstretch{1}
\section{Mapping the Shape of the U.S. Economy: A Topological Data
Analysis Approach with Ball
Mapper}\label{mapping-the-shape-of-the-u.s.-economy-a-topological-data-analysis-approach-with-ball-mapper}
...
Rate,\textsuperscript{\citeproc{ref-mcclung2024fisher}{\textbf{6}},
\citeproc{ref-michaelides2024fisher}{\textbf{7}}}\footnote{Approximation using the Fisher Equation.}
PPI-CPI
Gap,\textsuperscript{\citeproc{ref-bls_ppi_cpi_comparison}{\textbf{8}}}\footnote{Producer Price Index: Finished Goods less Consumer Price Index.}
and Real Wage
...
\caption{\label{tbl-Analysis_Dimensions}Variables inculded in Ball
Mapper Point Cloud}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{US_TDA_files/figure-pdf/fig-newGraph-1.pdf}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{US_TDA_files/figure-pdf/fig-newGraph-2.pdf}
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{US_TDA_files/figure-pdf/fig-EDA-1.pdf}
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{US_TDA_files/figure-pdf/fig-Fisher-1.pdf}
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{US_TDA_files/figure-pdf/fig-PPI-CPI_Gap-1.pdf}
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{US_TDA_files/figure-pdf/fig-Wage_Growth-1.pdf}
...
the end of the year and either had to give some back (negative wage
...
additional data that was excluded from out point cloud.}
...
first touches on the limitations we found both with Ball Mapper and the
design of our analysis. The second examines some of the unanswered
...
argument for a distance function, or hard code it into a copy of the
...
allow randomization of rows. They also have included a distance function
argument so a user can choose what metric they would like to use. For
instance, there are a variety of different metrics to choose from in the
\emph{scikit-learn} package for Python. Finally, \emph{pyBallMapper}
uses Python's NumPy package which utilizes vectorization, speeding up
the sequential nature of the Ball Mapper algorithm.

Second, we noticed during the analysis there is not a clear way to know
whether Ball Mapper is giving us a stable graph of our data. We
discussed that if given the same data, in the same order, it indeed will
give us the same graph. However, we did not find any literature
answering this exact question. The closest paper we could find was one
which studied the Brexit Vote using Ball Mapper. At the end of this
paper they wrote about a robustness test they implemented by showing why
...
For instance, in the Brexit example from the \emph{Discussion}, the
...
\CSLRightInline{Sullivan, D. G. (1997)
\href{https://www.chicagofed.org/publications/chicago-fed-letter/1997/march-115}{Trends
in real wage growth}, }
...
They graduated spring of 2018 with a Bachelor of Science in Mathematics.
During their senior year, Ryan was urged by their advisor Dr.~Mark Fitch
to pursue TDA after doing a presentation for a course and connected with
Dr.~Samuel Cook. Since then, they have continually worked with Dr.~Cook
outside their profession in social services. They will be continuing
their education at the University at Albany as a graduate student in the
Master's in Data Science program. This paper provided valuable
preparation and experience for the upcoming program's concentration
areas: TDA, machine learning, and statistics.

\section{PRESS SUMMARY}\label{press-summary}

Topological Data Analysis (TDA) is a data analysis method which is based
on the idea that any given data set has an inherent shape or structure
to it. Currently there is a large body of research that explores TDA's
most popular algorithm called Mapper; this paper uses a Mapper-adjacent
algorithm called Ball Mapper. The goals of this paper are to provide a
clear guide on how to use Ball Mapper, and to add to the small body of
literature on Ball Mapper's applications. We achieve these goals by
gathering data from the years 1960--2024 from the Bureau of Economic
Analysis (BEA), the Federal Reserve Bank (FRB), and the Bureau of Labor
Statistics (BLS). Our analysis found that Ball Mapper revealed distinct
types of economic activity: economic expansion, structural economic
peaks and troughs, and when the U.S. economy is in a contractionary
state or growing in an unstable, non-organic way. Further, within our
economic expansion finding, Ball Mapper showed that economic expansions
consist of various phases. Our results also did not use year as a
variable, which indicates that Ball Mapper graphs showed the underlying
structure of our dataset. Overall, this paper provides a concrete guide
of how to use Ball Mapper and shows potential to be useful for
cross-disciplinary analyses where qualitative, quantitative, or
high-dimensional data exist.
